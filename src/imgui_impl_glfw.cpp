...
//  2021-07-29: *BREAKING CHANGE*: Now using glfwSetCursorEnterCallback(). MousePos is correctly reported when the host platform g_window is hovered but not focused. If you called ImGui_ImplGlfw_InitXXX() with install_callbacks = false, you MUST install glfwSetWindowFocusCallback() callback and forward it to the backend via ImGui_ImplGlfw_CursorEnterCallback().
...
//  2019-03-12: Misc: Preserve DisplayFramebufferScale when main g_window is minimized.
...
void ImGui_ImplGlfw_NewFrame()
{
    ...
    // Setup display size (every frame to accommodate for g_window resizing)
    int w, h;
